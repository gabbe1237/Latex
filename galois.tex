...
\begin{mydef}{Normala delgrupper}{}
  ...
  alla $g \in G$ så är $ghg^{-1} \in N$. Vi skriver då $N \triangleleft G.$
\end{mydef}
Motivationen bakom denna till synes konstiga definition finner man i kvotgrupper. En definerande egenskap hos normala delgrupper tas upp i följande proposition.
...
\iff (a'b')N = (ab)N$, det vill säga att det inte spelar någon roll vilka representanter framför $N$, $a' \in aN, b' \in bN$, vi väljer, 
resultatet av operationen bör fortfarande vara i samma sidoklass som $ab$. Vi säger då att operationen är \textit{väldefinierad}.
 Om detta inte skulle gälla så skulle man kunna konstatera $G/N$ på flera olika sätt, vilket 
vi naturligtvist inte vill.  

Vi vet från del I att mängden av sidklasser utgör en partition av en grupp och därför kan vi rita följande figur.

\begin{center}
  \begin{tikzpicture}[->]
    \node at (0, 2.5) {$G$};
    \draw (-4,2) rectangle (4,-2);
    \foreach \i [count=\xi] in {-4, -2.4,...,0}{
    ...
    \node at (0, 0.5) {$b$};
    \node at (0, -0.5) {$b_1$};
    \node[scale=1.5] at (4.8, 0) {$\cdots$};
    \path[every node/.style={font=\sffamily\small}]
    (-1.6, 0.5) edge[bend right] node [left] {} (1, 1);
    \node at (0, -3.2) {$G/N = \{N, aN, bN, cN, (ab)N, \ldots\}$};
  \end{tikzpicture}
\end{center}
